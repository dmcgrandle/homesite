...
export class MediaService {
  ...
  public init(): void {
    this.sub = merge(
      this.readMedia(cfg.PHOTO_DIR.PATH, 'photo'),
      this.readMedia(cfg.VIDEO_DIR.PATH, 'video')
    ).subscribe(
      (mediaType): void =>
        console.log(new Date().toLocaleString(), ` : saved ${mediaType}Albums to db.`),
  ...
  private getMediaIds(medias: Media[], dirContents: FileObject[], tests: string[]): number[] {
    ...
    return mediasInDir.map((file): number => {
  ...
  private getAlbumIds(groupedFileObjects: FileObject[][], dirContents: FileObject[]): number[] {
    ...
    const result = dirsInDirContents.map((dir): number => {
  ...
  private ffmpeg$(...args: [string, string[], SpawnOptions?]): Observable<string> {
    ...
    return new Observable((observer): void => {
      ...
      ffmpeg.on('close', (returnCode): void => {
  ...
  private getRelTorPPath(file: string, mediaType: string): string {
    ...
    if (mediaType === 'photo') {
      url =
        uniqueFilename(path.join(cfg.PHOTO_DIR.PATH, cfg.PHOTO_DIR.CACHE_DIR), cfg.THUMBS.PREFIX, file) +
        cfg.THUMBS.SUFFIX;
    } else {
      url =
        uniqueFilename(path.join(cfg.VIDEO_DIR.PATH, cfg.VIDEO_DIR.CACHE_DIR), cfg.POSTERS.PREFIX, file) +
        cfg.POSTERS.SUFFIX;
    }
    return url;
  }
