...
test('test: create new token and verify', async (): Promise<void> => {
    expect.assertions(4);
...
test('test: modify token and verify failure', async (): Promise<void> => {
    expect.assertions(1);
    ...
    try {
        // make sure verify throws an error
...
test('test: create new email token and verify', async (): Promise<void> => {
    expect.assertions(4);
    try {
...
test('test: modify email token and verify failure', async (): Promise<void> => {
    expect.assertions(1);
    ...
    try {
        ...
        expect(err.message).toBe('403 Invalid token');
    }
});
